...
- **기능명:** 문장 기반 학습 뷰
- **설명:** 각 문장을 다음 버튼 혹은 캐러셀 형태로 넘기며 학습 가능
- **세부기능:**
  - 기본 원칙
    - **모바일 우선**. 사용자는 주로 휴대폰에서 가볍게 학습.
    - 단계적 노출(Progressive disclosure): 처음엔 가사 라인(간단) → 카드 클릭시 플립되면서 상세 내용 노출.
  ...
  - 동작 흐름(사용자 상호작용)
    1. 캐러셀에서 카드(Front) 표시.
    2. 사용자가 카드를 누르면 Flip 애니메이션 → Back 내용 노출.
    3. Back에서 한 번 더 누르면 front로 복귀
...
| 백엔드(API) | Node.js (Express) + AI API          |
